...
# 将训练样本拆分为训练集和验证集
train_set, test_set = features_added.randomSplit([0.7, 0.3])


# 构建线性回归模型，指定特征向量、预测标的与迭代次数
lr = LinearRegression(labelCol="SalePriceInt", featuresCol="features", maxIter=10)

# 使用训练集 train_set 训练线性回归模型
